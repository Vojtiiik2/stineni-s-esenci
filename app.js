    ...
    <>
      <header className={`site-header ${isScrolled ? "scrolled" : ""}`}>
        <div className="shell site-header-inner">
          <button type="button" className="brand" onClick={() => { go("/", { force: true }); setMenuOpen(false); }} aria-label={t.a11yHome}>
          ...
          <div className="hero-grid">
            ...
            <span className="script">{t.heroScript || t.brand2}</span>
              ...
              <div className="hero-actions">
                <button className="button button-primary" onClick={() => go("/contact")}>{t.cta}</button>
                <button type="button" className="button button-ghost" onClick={() => go("/gallery", { force: true })}>{t.galleryH}</button>
              </div>
            </div>
            {!isMobile && (
      ...
      <section className="section section-tight home-projects">
        <div className="shell">
          ...
          <div style={{ marginTop: 24 }} className="reveal home-projects-cta">
            <button type="button" className="button button-secondary" onClick={() => go("/gallery", { force: true })}>
              {t.galleryShowAll}
            </button>
          </div>
        </div>
