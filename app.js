    ...
    <>
      <header className={`site-header ${isScrolled ? "scrolled" : ""}`}>
        <div className="shell site-header-inner">
          <button className="brand" onClick={() => { go("/"); setMenuOpen(false); }} aria-label={t.a11yHome}>
  ...
  return (
    <>
      ...
      <section className="section">
        <div className="shell">
          <div className="section-header reveal pricing-header">
            {isMobile ? (
              <div className="pricing-mobile-intro">
                <h2 className="display h2 pricing-mobile-title">
                  {t.pricingSectionTitleMobile || t.pricingSectionTitle}
                </h2>

                <p className="lead pricing-mobile-lead">
                  {t.pricingIntroMobileLead || ((t.pricingIntroMobile || (t.pricingIntro || []).slice(0, 1))[0] || "")}
                </p>

                {!!(t.pricingMobilePoints || []).length && (
                  <ul className="pricing-mobile-points" aria-label={t.pricingMobilePointsLabel || "Pricing factors"}>
                    {(t.pricingMobilePoints || []).map((point, index) => (
                      <li key={index}>{point}</li>
                    ))}
                  </ul>
                )}

                {t.pricingIntroMobileNote && (
                  <p className="pricing-mobile-note">{t.pricingIntroMobileNote}</p>
                )}
              </div>
            ) : (
              <>
